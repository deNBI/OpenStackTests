...
try:
    logger.info("----------------------")
    logger.info("Start Test: check_cinderv2")
    logger.info("Trying to connect to Cinderv2-Endpoint... ")
    ...
    r=requests.get(CINDER_ENDPOINTv2,headers=headers)
    if(str(r.status_code)[0] == '2' ):
        logger.info("Succesful Test: check_cinderv2")
        logger.info("----------------------")
        sys.exit(0)
    else:
        logger.error(str(r.status_code) + ": " + str(r.content)[:97])
        logger.info("Failed Test: check_cinderv2")
        logger.info("----------------------")
