...
try:
    with open('config.yml','r') as ymlfile:
        ...
        NOVA_ENDPOINT=cfg['endpoints']['nova_endpoint']
...
try:
    logger.info("----------------------")
    logger.info("Start Test: check_nova")
    logger.info("Trying to connect to Nova-Endpoint... ")
    ...
    r = requests.get(NOVA_ENDPOINT,headers=headers)
    if (str(r.status_code)[0] == '2'):
        logger.info("Nova alive ...")
        logger.info("Succesful Test: check_nova")
        logger.info("----------------------")
        sys.exit(0)
    else :
